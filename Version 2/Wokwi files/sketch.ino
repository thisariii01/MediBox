...
void loop() {
    ...
    mqttClient.loop();
		...
		updateHumidity();
		updateLDR();
    mqttClient.publish("200023C-TEMP", tempAr);
		mqttClient.publish("200023C-HUMIDITY", humidAr);
    mqttClient.publish("2000023C-INTENSITY", ldrAr);
    
    checkSchedule();
    delay(1000);
    theta = minAngle + (180 - minAngle) * LDR_val * ctrlFactor;
    servo.write(theta);
    delay(50);
}
...
void receiveCallback(char *topic, byte *payload, unsigned int length) {
    ...
    Serial.println();
		...
		if(strcmp(topic,"200023C-MAIN-ON-OFF") == 0) {
          buzzerOn(payloadCharAr[0]=='1');  
    }
		else if(strcmp(topic,"200023C-SCH-ON") == 0) {
    ...
    else if (strcmp(topic, "200023C-MIN-ANGLE") == 0) {
        minAngle = atoi(payloadCharAr);
    }
    else if (strcmp(topic, "200023C-CTRL-FACTOR") == 0) {
...
void connectToBroker() {
    while (!mqttClient.connected()) {
        ...
        if (mqttClient.connect("ESP32-200023C")) {
            Serial.println("connected");
						//Subscribing To Topics
						mqttClient.subscribe("200023C-MAIN-ON-OFF");
						mqttClient.subscribe("200023C-SCH-ON"); 
            mqttClient.subscribe("200023C-MIN-ANGLE");
            mqttClient.subscribe("200023C-CTRL-FACTOR");
        }
        else {
            Serial.print("failed");
...
//Function to update temperature to Array
void updateTemperature() {
    TempAndHumidity data = dhtSensor.getTempAndHumidity();
    String(data.temperature, 2).toCharArray(tempAr, 6);
}

//Function to update humidity to Array
void updateHumidity() {
    TempAndHumidity data = dhtSensor.getTempAndHumidity();
    String(data.humidity, 2).toCharArray(humidAr, 6);
}

//Function to update LDR value to Array
void updateLDR() {
    LDR_val = 1 - analogRead(LDRpin)/4063.0;
		String(LDR_val, 2).toCharArray(ldrAr, 6);
}
...
//Function to check the schedule
void checkSchedule() {
    if(isScheduled) {
        ...
        if(currentTime > scheduleOnTime) {
            ...
            mqttClient.publish("200023C-MAIN-ON-OFF-ESP", "1");
            mqttClient.publish("200023C-SCH-ESP-ON", "0");
        }
    }
}
